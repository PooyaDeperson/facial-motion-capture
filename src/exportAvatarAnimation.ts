...
export function exportAvatarAnimation(headMesh: THREE.Mesh[], nodes: any, frames: any[], fileName = "avatarRecording.glb") {
  ...
  // Create a dummy scene for export
  ...
  exporter.parse(scene, result=>{
    const blob = result instanceof ArrayBuffer ? new Blob([result], {type:"model/gltf-binary"}) : new Blob([JSON.stringify(result,null,2)], {type:"application/json"});
